...
router.patch('/strategies/:id', async (req, res) => {
  ...
  try {
    const strategy = await yieldOptimizerService.updateStrategy(
      req.params.id,
      actor,
      body
    );
...
router.post('/strategies/:id/deposit', async (req, res) => {
  ...
  try {
    const result = await yieldOptimizerService.deposit(
      req.params.id,
      actor,
      amount
    );
...
router.post('/strategies/:id/withdraw', async (req, res) => {
  ...
  try {
    const result = await yieldOptimizerService.withdraw(
      req.params.id,
      actor,
      amount
    );
...
router.post('/backtest', async (req, res) => {
  const body = {
    strategyId:
      req.body?.strategyId != null ? Number(req.body.strategyId) : null,
  ...
  if (body.strategyId == null) {
    ...
    if (
      !Number.isFinite(body.compoundEveryDays) ||
      body.compoundEveryDays <= 0
    ) {
      errors.push(
        'compoundEveryDays must be provided when strategyId is omitted'
      );
    }
  }
