...
function calcBuyPrice(pool) {
  ...
  const protocolFee = Math.floor(
    (pool.spotPrice * store.protocolFeeBps) / BPS_DENOM
  );
...
function nextPriceDown(pool) {
  ...
  if (pool.curve === 'Linear') {
    ...
    newPrice =
      pool.spotPrice - Math.floor((pool.spotPrice * pool.delta) / BPS_DENOM);
  }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (poolType && validatePoolType(poolType))
      pools = pools.filter((p) => p.poolType === poolType);
    if (curve && validateCurve(curve))
      pools = pools.filter((p) => p.curve === curve);
    if (collection) pools = pools.filter((p) => p.nftCollection === collection);
    if (active !== undefined)
      pools = pools.filter((p) => p.active === (active === 'true'));
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!validateAddress(nftCollection))
      return next(createHttpError(400, 'Invalid nftCollection address'));
    if (!validateAddress(paymentToken))
      return next(createHttpError(400, 'Invalid paymentToken address'));
    if (!validateCurve(curve))
      return next(
        createHttpError(400, `curve must be one of: ${CURVE_TYPES.join(', ')}`)
      );
    if (!validatePoolType(poolType))
      return next(
        createHttpError(
          400,
          `poolType must be one of: ${POOL_TYPES.join(', ')}`
        )
      );

    const spot = parseInt(spotPrice, 10);
    if (!Number.isInteger(spot) || spot <= 0)
      return next(createHttpError(400, 'spotPrice must be a positive integer'));

    const d = parseInt(delta, 10);
    if (!Number.isInteger(d) || d < 0)
      return next(createHttpError(400, 'delta must be a non-negative integer'));

    const fee = parseInt(feeBps, 10);
    if (!Number.isInteger(fee) || fee < 0 || fee > MAX_FEE_BPS)
      return next(createHttpError(400, `feeBps must be 0–${MAX_FEE_BPS}`));
    if (poolType !== 'Trade' && fee > 0)
      return next(createHttpError(400, 'Only Trade pools can have a fee'));
    ...
    res
      .status(201)
      .json({ success: true, message: 'Pool created', data: pool });
  })
);
...
router.patch(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!pool) return next(createHttpError(404, `Pool #${id} not found`));
    if (pool.owner !== owner)
      return next(createHttpError(403, 'Not the pool owner'));
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (pool.owner !== owner)
      return next(createHttpError(403, 'Not the pool owner'));
    if (!pool.active) return next(createHttpError(409, 'Pool is not active'));
    if (pool.poolType === 'Sell')
      return next(createHttpError(409, 'Sell pools do not hold tokens'));
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!Array.isArray(nftIds) || nftIds.length === 0)
      return next(createHttpError(400, 'nftIds must be a non-empty array'));
    const pool = store.pools.get(id);
    if (!pool) return next(createHttpError(404, `Pool #${id} not found`));
    if (pool.owner !== owner)
      return next(createHttpError(403, 'Not the pool owner'));
    if (!pool.active) return next(createHttpError(409, 'Pool is not active'));
    if (pool.poolType === 'Buy')
      return next(createHttpError(409, 'Buy pools do not hold NFTs'));
    pool.nftIds.push(...nftIds);
    pool.nftCount = pool.nftIds.length;
    res.json({
      success: true,
      message: `Deposited ${nftIds.length} NFTs`,
      data: pool,
    });
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!amt)
      return next(createHttpError(400, 'amount must be a positive integer'));
    if (amt > pool.tokenBalance)
      return next(createHttpError(409, 'Insufficient token balance'));
    pool.tokenBalance -= amt;
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!n)
      return next(createHttpError(400, 'count must be a positive integer'));
    if (n > pool.nftCount)
      return next(createHttpError(409, 'Insufficient NFTs in pool'));
    const withdrawn = pool.nftIds.splice(pool.nftIds.length - n, n);
    pool.nftCount = pool.nftIds.length;
    res.json({
      success: true,
      message: `Withdrew ${n} NFTs`,
      data: { pool, withdrawnNftIds: withdrawn },
    });
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!validateAddress(buyer))
      return next(createHttpError(400, 'Invalid buyer address'));
    const pool = store.pools.get(id);
    if (!pool) return next(createHttpError(404, `Pool #${id} not found`));
    if (!pool.active) return next(createHttpError(409, 'Pool is not active'));
    if (pool.poolType === 'Buy')
      return next(createHttpError(409, 'Buy pools do not sell NFTs'));
    if (pool.nftCount === 0)
      return next(createHttpError(409, 'No NFTs in pool'));

    const totalPrice = calcBuyPrice(pool);
    const max = parseInt(maxPrice, 10);
    if (!Number.isInteger(max) || totalPrice > max) {
      return next(
        createHttpError(409, `Price ${totalPrice} exceeds maxPrice ${max}`)
      );
    }
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!validateAddress(seller))
      return next(createHttpError(400, 'Invalid seller address'));
    if (nftId === undefined || nftId === null)
      return next(createHttpError(400, 'nftId is required'));
    const pool = store.pools.get(id);
    if (!pool) return next(createHttpError(404, `Pool #${id} not found`));
    if (!pool.active) return next(createHttpError(409, 'Pool is not active'));
    if (pool.poolType === 'Sell')
      return next(createHttpError(409, 'Sell pools do not buy NFTs'));

    const payout = calcSellPrice(pool);
    const min = parseInt(minPrice, 10);
    if (!Number.isInteger(min) || payout < min) {
      return next(
        createHttpError(409, `Payout ${payout} is below minPrice ${min}`)
      );
    }
    ...
    if (pool.tokenBalance < totalCost) {
      return next(
        createHttpError(
          409,
          `Insufficient pool token balance (${pool.tokenBalance} < ${totalCost})`
        )
      );
    }
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    res.json({
      success: true,
      data: { buyPrice: calcBuyPrice(pool), spotPrice: pool.spotPrice },
    });
  })
);
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    res.json({
      success: true,
      data: { sellPrice: calcSellPrice(pool), spotPrice: pool.spotPrice },
    });
  })
);
...
router.get(
  '/collections/:address/analytics',
  asyncHandler(async (req, res) => {
    const { address } = req.params;
    const collectionPools = [...store.pools.values()].filter(
      (p) => p.nftCollection === address
    );
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    res.json({
      success: true,
      message: paused ? 'AMM paused' : 'AMM unpaused',
      data: { paused },
    });
  })
);
  ...
  asyncHandler(async (_req, res) => {
    res.json({
      ...
      data: {
        protocolFeeBps: store.protocolFeeBps,
        protocolFeeBalance: store.protocolFeeBalance,
        protocolFeeBalanceXlm: (store.protocolFeeBalance / 10_000_000).toFixed(
          7
        ),
      },
    });
