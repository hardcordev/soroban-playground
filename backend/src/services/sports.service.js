...
const getMarketOdds = async (id) => {
  ...
  const totalStake = market.outcomes.reduce(
    (acc, curr) => acc + curr.total_stake,
    0
  );
  return market.outcomes.map((outcome) => ({
    name: outcome.name,
    odds:
      totalStake > 0 ? (totalStake / outcome.total_stake).toFixed(2) : '2.00',
  }));
};

const getGlobalAnalytics = async () => {
  const totalVolume = markets.reduce(
    (acc, m) => acc + m.outcomes.reduce((a, o) => a + o.total_stake, 0),
    0
  );
